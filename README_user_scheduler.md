...
- 默认监控 CPU 和 GPU：`--sensors CPU,GPU`
...
```bash
python3 omen_k0000_fan_controller.py --dry-run --once --simulate CPU=82
```
...
```bash
sudo omen-k0000-fan-controller --profile performance --sensors CPU,GPU --gpu-temp-policy active-only
```

可选：同时切到内核平台性能配置：

```bash
sudo omen-k0000-fan-controller --profile performance --sensors CPU,GPU --gpu-temp-policy active-only --platform-profile performance
```

默认服务已经让 GPU 温度参与调度，同时不唤醒 D3 状态的 dGPU：

```bash
sudo omen-k0000-fan-controller --sensors CPU,GPU --gpu-temp-policy active-only
```
...
- `--sensors CPU,GPU`: 选择监控传感器；可用 `CPU,GPU,IR` 或 `ALL`，默认监控 CPU 和 GPU。
